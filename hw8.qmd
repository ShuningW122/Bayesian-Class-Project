...
> Do effort preferences on rewarded trials in the training section differ between the effort and performance conditions? (model1) Do effort preferences on rewarded trials in the training section differ between the effort and neutral conditions? (model2) Do effort preferences on probe (unrewarded) trials in the training section differ between the effort and performance conditions? (model3) Do effort preferences on probe (unrewarded) trials in the training section differ between the effort and neutral conditons? (model4) Do effort preferences on the dot-motion task in the post-training section differ between the effort and performance conditions? (model5) Do effort preferences on the dot-motion task in the post-training section differ between the effort and neutral conditions? (model6) Do effort preferences on the math task in the post-training section differ between the effort and performance conditions? (model7) Do effort preferences on the math task in the post-training section differ between the effort and neutral conditions? (model8)
...
$$
Y_i \sim \text{Bin}(N_{\text{reward}_i}, \mu_i)
$$
...
$$
\eta_j = \beta_0 + \beta_1 (\text{condition}_i) + \beta_2 (\text{pre_training}_i)
$$
